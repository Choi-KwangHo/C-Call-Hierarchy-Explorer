...
class AppIntegrationTests(unittest.TestCase):
    ...
    def test_file_path_compacts_with_panel_width(self) -> None:
        ...
        self.assertEqual(APP_VERSION, "1.5.10")
        window.close()
    ...
    def test_vscode_style_project_settings_and_exclusion_normalization(self) -> None:
        with tempfile.TemporaryDirectory() as temporary:
            ...
            self.assertEqual(dialog.categories.item(0).text(), "프로젝트 분석 범위")
            self.assertEqual(dialog.categories.item(1).text(), "EEPROM 소스 및 동기화")
            self.assertGreaterEqual(dialog.categories.item(0).sizeHint().height(), 40)
            dialog.search.setText("EEPROM")
            self.assertEqual(dialog.categories.currentRow(), 1)
            ...
            self.assertNotIn("EEPROM", menu_titles)
    ...
    def test_code_preview_distinguishes_parent_children_and_external_calls(self) -> None:
        with tempfile.TemporaryDirectory() as temporary:
            ...
            self.assertTrue({"#1b5574", "#165264", "#68481a"}.issubset(colors))
            window.close()
